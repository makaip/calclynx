...
class TextFieldProseMirror {
	...
	createProseMirrorEditor(content = '') {
		...
		//todo: fix the unreadable indenting here
				...
				plugins: [
					...
					keymap({
						'$': (state, dispatch) => {
							...
							if (dispatch) {
								const tr = state.tr.replaceSelectionWith(this.schemaManager.createMathNode(""));
								dispatch(tr);
		...
		if (this.resizeHandler) this.resizeHandler.createResizeHandles(this.container);
	...
	remove() {
		...
		if (this.proseMirrorView) this.proseMirrorView.destroy();
	...
	setWidthData(data) {
		if (this.resizeHandler) this.resizeHandler.setWidthData(data);
	}
}
